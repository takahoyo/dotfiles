...
if [[ -n "${REMOTEHOST}${SSH_CONNECTION}" ]]; then
	local lip=`echo -n ${SSH_CONNECTION} | cut -d " " -f1`
	local rip=`echo -n ${SSH_CONNECTION} | cut -d " " -f3`
	local p_host="%F{yellow}$lip -> $rip%f"
else
	local lhost=`who am i|sed 's/.*(\(.*\)).*/\1/'`
	local rhost=`echo -n ${HOSTNAME}`
	local p_host="%F{yellow}$lhost -> $rhost%f"
fi
